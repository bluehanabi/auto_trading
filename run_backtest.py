#!/usr/bin/env python3
"""한국 주식 '1% 익절 + 손절' 전략 백테스트 실행기.

사용 예시
  # 실데이터 (네트워크 필요, FinanceDataReader 로 KRX 일봉 다운로드)
  python3 run_backtest.py --symbol 005930 --start 2020-01-01 --end 2024-12-31

  # 합성 데이터로 즉시 검증 (오프라인 환경)
  python3 run_backtest.py --synthetic --entry everyday

  # 손절선을 바꿔 가며 비교 — '1% 익절'의 함정을 한눈에
  python3 run_backtest.py --synthetic --entry everyday --sweep

  # 손절 없는 '함정' 버전
  python3 run_backtest.py --synthetic --entry everyday --no-stop
"""
import argparse
...
def print_report(df, m, args, bh_return):
    ...
    if m["num_trades"] == 0:
        ...
        print("=" * 64)
        return
    print(f" 총 매매 횟수         {m['num_trades']:>6,} 회")
    print(f" 승률                 {m['win_rate'] * 100:>6.1f} %")
    print(f" 평균 수익(이긴 매매) {_pct(m['avg_win'])}")
    print(f" 평균 손실(진 매매)   {_pct(m['avg_loss'])}")
    print(f" 손익비(payoff)       {m['payoff']:>7.2f}   (1 미만이면 구조적으로 불리)")
    print(f" Profit Factor        {m['profit_factor']:>7.2f}   (1 미만이면 손실)")
    ...
    print(f" 평균 보유일          {m['avg_hold_days']:>6.1f} 일")
    reasons = m["reasons"]
    rtxt = " / ".join(f"{k} {v}" for k, v in sorted(reasons.items()))
    print(f" 청산 사유            {rtxt}")
    print("=" * 64)
    _verdict(m, bh_return)
    if args.tp <= 0.02:
        print(" * 주의: 일봉은 장중 가격 경로를 알 수 없어 ±1%처럼 좁은 폭은")
        print("   갭·노이즈에 민감합니다. 정확히 보려면 분봉 데이터로 재검증하세요.")
    print("=" * 64)


def _verdict(m, bh_return):
    win = m["win_rate"] * 100
    if m["total_return"] <= 0:
        print(f" ▶ 승률 {win:.0f}% 인데도 '손실'. 작은 익절을 큰 손실과 잦은")
        print(f"   거래비용({m['total_cost']:,.0f}원)이 상쇄 — '1% 익절'의 전형적 함정.")
    elif m["total_return"] < bh_return:
        print(f" ▶ 수익은 났지만 단순 매수후보유({_pct(bh_return)})보다 부진.")
        print(f"   잦은 매매로 추세 상승분을 거의 놓침.")
    else:
...
def run_sweep(df, args):
    ...
    sl_list = [0.005, 0.01, 0.02, 0.03, None]
    ...
    print("=" * 64)
    print(f" {'손절':>8} | {'승률':>7} | {'손익비':>7} | {'매매수':>7} | {'총수익률':>10}")
    print("-" * 64)
    for sl in sl_list:
        res = run_backtest(
            df, sig, take_profit=args.tp, stop_loss=sl,
            max_hold_days=args.max_hold, initial_cash=args.cash,
            commission=args.commission, tax=args.tax, slippage=args.slippage,
        )
        m = compute_metrics(res)
        sl_txt = "없음" if not sl else f"-{sl * 100:.1f}%"
        print(f" {sl_txt:>8} | {m['win_rate'] * 100:>6.1f}% | "
              f"{m['payoff']:>7.2f} | {m['num_trades']:>7,} | "
              f"{m['total_return'] * 100:>+9.2f}%")
    print("=" * 64)
    print(" ▶ 손절을 늘리면 승률은 오르지만 한 번의 손실이 커진다.")
    print("   '손절 없음'은 승률이 가장 높아도 큰 손실(꼬리위험)을 떠안는다.")
...
def main():
    p = argparse.ArgumentParser(
        description="한국 주식 '1% 익절' 전략 백테스트",
    ...
    p.add_argument("--entry", default="everyday", choices=list(ENTRY_SIGNALS),
                   help="진입 신호")
    p.add_argument("--tp", type=float, default=config.TAKE_PROFIT, help="익절 비율")
    p.add_argument("--sl", type=float, default=config.STOP_LOSS, help="손절 비율")
    p.add_argument("--no-stop", action="store_true", help="손절 비활성화")
    p.add_argument("--max-hold", type=int, default=config.MAX_HOLD_DAYS,
                   help="최대 보유 거래일")
    ...
    try:
        df = load_data(args.symbol, args.start, args.end, synthetic=args.synthetic)
